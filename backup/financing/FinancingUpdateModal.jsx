...
const FinancingUpdateModal = ({ financingData, onUpdate, disabled }) => {
  ...
  const [interes, setInteres] = useState(formState.interes);
  const [monto, setMonto] = useState(formState.monto);
  const [cuota, setCuota] = useState(formState.cuota);
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid style={{ display: 'flex', justifyContent: 'flex-end' }} item xs={12}>
                    <Button
                      ...
                      variant="contained"
                      onClick={() => {
                        setCuota(formState.cuota);
                        setMonto(formState.monto);
                        setInteres(formState.interes);
                        onResetForm();
                      }}
                      className="ml-2"
                    >
